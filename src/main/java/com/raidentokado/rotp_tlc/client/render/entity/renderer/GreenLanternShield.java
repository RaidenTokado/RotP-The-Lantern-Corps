...
package com.raidentokado.rotp_tlc.client.render.entity.renderer;
...
public class GreenLanternShield<T extends LivingEntity, M extends PlayerModel<T>> extends LayerRenderer<T, M> {
    private static final Map<PlayerRenderer, GreenLanternShield<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>> RENDERER_LAYERS = new HashMap<>();
    private final M thornsModel;
    private final boolean slim;
    private boolean playerAnimHandled = false;

    public GreenLanternShield(IEntityRenderer<T, M> renderer, M hermitoModel, boolean slim) {
        super(renderer);
        if(renderer instanceof PlayerRenderer){
            RENDERER_LAYERS.put((PlayerRenderer) renderer,(GreenLanternShield<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>) this);
        }
    ...
    public void render(MatrixStack matrixStack, IRenderTypeBuffer buffer, int packedLight, T entity,
        ...
        IStandPower.getStandPowerOptional(entity).ifPresent((stand)->{
            ...
            if(stand.getType() == hm && stand.getStandManifestation()instanceof StandEntity && stand.getHeldAction()==InitStands.GREEN_LANTERN_BLOCK.get()){
                ...
                ResourceLocation texture = new  ResourceLocation(RotpTLCAddon.MOD_ID,"/textures/entity/stand/green_lantern_shield"+(slim ? "_slim" : "") + ".png");
