...
public class GLShieldEntity extends Entity implements IEntityAdditionalSpawnData {
    ...
    public GLShieldEntity(World world, @Nonnull LivingEntity hamonUser, float width, float height) {
        this(InitEntities.GL_SHIELD.get(), world);
        this.user = hamonUser;
        this.power = IStandPower.getStandPowerOptional(hamonUser).orElse(null);
        this.width = width;
        this.height = height;
        refreshDimensions();
    }

    public GLShieldEntity(EntityType<?> type, World world) {
        super(type, world);
    }
    ...
    public void tick() {
        ...
        if (user == null || !user.isAlive() || !level.isClientSide() &&
                (power == null || power.getHeldAction() != InitStands.GREEN_LANTERN_BLOCK.get())) {
    ...
    public void readSpawnData(PacketBuffer additionalData) {
        ...
        if (entity instanceof LivingEntity) {
            ...
            this.power = IStandPower.getStandPowerOptional(user).orElse(null);
        }
    }
